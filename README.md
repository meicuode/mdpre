...
1. **纯前端解析**
   ...
   - **表格宽度切换**：鼠标悬停在表格上时右上角出现「全宽/标准」切换按钮组。全宽模式下表格占满整个可视窗口宽度，标准模式下表格按内容实际宽度显示。滚动时表头自动固定在顶部。
   - **锚点跳转**：所有标题自动生成 `id` 属性（支持中文），点击标题左侧的 `#` 锚链接或文档内 `[链接](#标题)` 可平滑滚动到对应章节。长距离跳转采用"瞬移 + 短滑"策略，兼顾速度与视觉反馈。
   - **文档大纲**：页面右侧自动展示文档目录大纲（类似语雀），支持多级标题缩进、当前章节高亮跟踪（IntersectionObserver）、点击跳转、折叠/展开切换。
   - **快速滚动**：页面右下角提供半透明的「回到顶部 ↑」和「跳到底部 ↓」按钮，鼠标悬停时透明度增强。
   - **任务列表**：支持 `- [x]` / `- [ ]` 语法渲染为带复选框的任务列表。
   - **文档审阅引用**：选中文档中的文字后，选区旁弹出「📎 引用」按钮，点击后自动提取章节路径和选中文字，追加到左下角的审阅面板中。面板支持拖拽移动、拖拽调整大小、多次引用累加、手动编辑改进建议、一键复制全部内容、清空。最小化后在左下角显示审阅图标，点击即可恢复。
   - **阅读状态记忆**：自动记住每个文档的滚动位置、审阅面板的打开状态/位置/大小、审阅内容。下次打开或刷新页面时自动恢复上次的阅读状态。
